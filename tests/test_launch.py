...
from betterscale.models import dsv4_draft
...
def config():
    return NS(
        ...
        scheduler_config=NS(
            max_num_seqs=4, max_num_batched_tokens=4128, scheduler_cls=None
        ),
        model_config=NS(
            hf_config=NS(
...
class LaunchContract(unittest.TestCase):
    ...
    def test_capacity_is_not_set_by_the_patch(self):
        ...
        c = config()
        c.cache_config.kv_cache_memory_bytes = 1024
        validate_worker_config(c)
    ...
    def test_reject_unqualified_parallelism_and_scheduler(self):
        for field, value in [
            ...
            with self.assertRaisesRegex(ValueError, "qualified"):
                validate_worker_config(c)
        c = config()
        c.scheduler_config.scheduler_cls = "experimental.N2"
        with self.assertRaisesRegex(ValueError, "native scheduler"):
            validate_worker_config(c)

    def test_reject_unqualified_request_or_draft_capacity(self):
        c = config()
        c.scheduler_config.max_num_seqs = 8
        with self.assertRaises(ValueError):
            validate_worker_config(c)
        c = config()
        c.speculative_config.num_speculative_tokens = 3
        with self.assertRaises(ValueError):
            validate_worker_config(c)
    ...
    def test_fail_closed_on_changed_private_api(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            with (
                patch("betterscale.compat.pins", return_value=manifest),
                patch("betterscale.compat.metadata.version", return_value="1.0+build"),
                patch(
                    "betterscale.compat.metadata.distribution",
                    return_value=NS(locate_file=lambda p: root / p),
                ),
            ):
                check_runtime.cache_clear()
                self.assertEqual(check_runtime()["checked_source_files"], 1)
...
class WorkerLifecycle(unittest.TestCase):
    def worker_class(self, calls):
        ...
        node = next(
            n for n in ast.parse(source.read_text()).body if isinstance(n, ast.ClassDef)
        )
        ...
        class Native:
            ...
            def compile_or_warm_up_model(self):
                calls.append("native-capture")
                return "native-times"

        from threading import RLock
        from betterscale.models import dsv4
        from betterscale.patches import auto_kv

        def select(c):
            return dsv4, dsv4.validate(c)

        self.enterContext(
            patch.object(dsv4, "check_runtime", lambda: calls.append("compat"))
        )
        self.enterContext(
            patch.object(dsv4, "log", NS(info=lambda *a, **kw: calls.append("ready")))
        )
        self.enterContext(
            patch.object(auto_kv, "snapshot", lambda w, phase: calls.append(phase))
        )
        self.enterContext(
            patch.object(
                dsv4_draft, "prepare_final", lambda w: calls.append("memory-ready")
            )
        )
        ns = dict(
            __name__="betterscale.worker_test",
            NPUWorker=Native,
            select=select,
            _lock=RLock(),
            _route=None,
    ...
    def test_only_worker_cls_is_needed_and_native_configuration_is_preserved(self):
        ...
        with (
            ...
            patch(
                "pathlib.Path.mkdir",
                side_effect=AssertionError("No artifact directory"),
            ),
            patch(
                "pathlib.Path.write_text",
                side_effect=AssertionError("No receipt files"),
            ),
        ):
...
class DPLifecycle(WorkerLifecycle):
    def test_native_dp_composition_excludes_tp_draft_and_experiment_hooks(self):
        ...
        with (
            patch.dict(sys.modules, modules),
            patch("pathlib.Path.write_text", side_effect=AssertionError("No receipts")),
        ):
            worker = self.worker_class(calls)(c)
            self.assertEqual(worker.compile_or_warm_up_model(), "native-times")
        self.assertEqual(c, before)
