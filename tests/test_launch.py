...
class LaunchContract(unittest.TestCase):
    ...
    def test_fail_closed_on_changed_private_api(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            with (
                patch("strengthen_dsv4.compat.pins", return_value=manifest),
                patch(
                    "strengthen_dsv4.compat.metadata.version", return_value="1.0+build"
                ),
                patch(
                    "strengthen_dsv4.compat.metadata.distribution",
...
class WorkerLifecycle(unittest.TestCase):
    def worker_class(self, calls):
        ...
        ns = dict(
            __name__="strengthen_dsv4.worker_test",
            __package__="strengthen_dsv4",
            NPUWorker=Native,
            check_runtime=lambda: calls.append("compat"),
            validate_worker_config=validate_worker_config,
            PATCH_IDS=PATCH_IDS,
            DP_PATCH_IDS=DP_PATCH_IDS,
    ...
    def test_only_worker_cls_is_needed_and_native_configuration_is_preserved(self):
        ...
        for name in (
            ...
            if name in ("ordered_replay", "async_decode"):
        ...
        self.assertEqual(
            ...
            [
                ...
                "compat_lcm",
                "target_full",
                "ordered_replay-capture",
                "async_decode-capture",
                "native-init",
                "native-capture",
                "split_draft",
                "cross_step",
