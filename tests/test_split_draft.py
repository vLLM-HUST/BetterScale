...
class SplitDraftTests(unittest.TestCase):
    def test_context_hook_restored_after_success_and_failure(self):
        ...
        ).joinpath("split_draft/__init__.py")
        node = next(
            n
    ...
    def test_one_manager_owns_decode_and_query_routes(self):
        ...
        path = (
            ...
            / "src/strengthen_dsv4/patches/split_draft/__init__.py"
        )
        nodes = [
            n
            for n in ast.parse(path.read_text()).body
        ...
        d = SimpleNamespace(
            ...
            _dflash_num_context=6,
            _context_slot_mapping_buffers=[],
            build_model_inputs_first_pass=ingest,
        )
        metadata = {
            "a": SimpleNamespace(
                num_prefills=0, attn_state=SimpleNamespace(name="mode")
            )
        }
        ...
        class Entry:
            def __init__(self, worker, original, count, pool=None):
                self.pool = pool
        ...
        ns = dict(
            torch=SimpleNamespace(
                npu=SimpleNamespace(graph_pool_handle=lambda: "shared-draft-pool")
            ),
            contextmanager=contextmanager,
            record_function=nullcontext,
            ExactDraftGraph=Entry,
            get_forward_context=lambda: ctx,
            graph_metadata=lambda value: dict(value),
        )
        exec(compile(ast.Module(body=nodes, type_ignores=[]), str(path), "exec"), ns)
        manager = ns["DraftGraphRunner"](worker)
        self.assertEqual(manager(batch_size=1), "native")
        self.assertFalse(manager.decode_graphs)
