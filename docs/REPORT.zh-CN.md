...
交付物不再只有原型：维护源码在 `src/strengthen_dsv4/`。当前只保留一个入口：
在既有 vLLM-Ascend 环境安装包，给原生 `vllm serve` 加
`--worker-cls strengthen_dsv4.worker.Worker`。不接管环境或另造服务启动器。
...
旧版交付曾通过真实 HTTP 验收：经当时的自定义 CLI 启动原生服务，`optimized` 与
...
当前仅 worker 入口版本不做 shadow、不复制整个 KV 池做验证，也不导入历史 N+2 原型。
它不要求 artifacts 目录或生产 JSON 收据；安装信息进入正常日志。旧验收摘要仍指向
旧 CLI，不作为新版入口重新跑过的真权重声明。

## 5. 如何复现和回退

在用户已有、已经能运行 donor 的环境安装本包，不升级依赖，不重建算子：

```bash
python -m pip install --no-deps --no-build-isolation /path/to/strengthen-dsv4
vllm serve /models/DeepSeek-V4-Flash <原有原生参数> \
  --worker-cls strengthen_dsv4.worker.Worker
```

上面的原生参数是占位说明，完整示例见[运行手册](RUNBOOK.zh-CN.md)。
