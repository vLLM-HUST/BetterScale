...
### TP 近满池 dummy 验收（run189）

同一共享池自动定容程序继续完成四个448Ki输入／2048输出请求，再完成16个8K输入／128输出
的周转请求。原生KV占用达到**96.84%**；最多三个长请求同时运行，第四个排队，不能宣称
四个长上下文同时驻留。八个rank各提交3859波，draft fallback全为0，运行期额外分配峰值
约326.48MiB/rank。没有发生抢占，早前3GiB预算下的恢复失败仍未修复。

见`evidence/auto-kv-run189.json`。这里只验容量、执行与回收，不计作真权重吞吐或质量成绩；
新TP启动组合还需要独立的真权重质量门，公共包与默认配置尚未更新。
