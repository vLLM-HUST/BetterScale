...
# Pinned Ascend 9bf964cb WY math. Experimental head-parallel scheduling
# and optional kernel-local inverse; retain FP32 dots / BF16 inverse boundary.
# ruff: noqa: E501
# mypy: ignore-errors

from vllm.triton_utils import tl, triton
from solve_fusion import inverse64, inverse_from_ad
...
def solve_wy_kernel(
    ...
    FUSE_SOLVE: tl.constexpr,
    FUSE_MERGE: tl.constexpr,
    HEADS_PER_TASK: tl.constexpr,
):
    T_max = T
    i_t_o = tl.program_id(0)

    for i_bh in range(tl.program_id(1) * HEADS_PER_TASK, (tl.program_id(1) + 1) * HEADS_PER_TASK):
        ...
        if i_t * BT < T:
            ...
            mask_A = mask_t[:, None]
            if FUSE_SOLVE:
                # Preserve the original BF16 Ai rounding before FP32 WY dot.
                b_A = inverse64(A, bos, T, i_t, i_h, H).to(k.dtype.element_ty).to(tl.float32)
            elif FUSE_MERGE:
                b_A = inverse_from_ad(A, Ad, bos, T, i_t, i_h, H).to(k.dtype.element_ty).to(tl.float32)
            else:
