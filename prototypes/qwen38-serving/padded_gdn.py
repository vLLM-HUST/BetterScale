"""Experimental single-sequence padding: virtual GDN tokens are identity updates.

Keep causal-convolution endpoints real. Only chunk recurrence consumes a padded
sequence, with q/k/v/beta/g zeroed outside the real tensor endpoint. This needs
real state/output parity evidence before adoption; mixed batches are excluded.
"""
...
def normalize(builder, metadata, bucket):
    ...
    if bucket not in builder._padded_chunks:
        ...
        chunk = _build_non_spec_chunked_prefill_metadata(builder, cpu, metadata.prefill_query_start_loc.device)
