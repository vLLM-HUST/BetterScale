"""Same-pair ABBA launcher. Admission owns the subset lease and process group."""
...
def main():
    ...
    try:
        for index, arm in enumerate(("baseline", "candidate", "candidate", "baseline")):
            ...
            reclaimed(root)
            env = os.environ.copy()
            env.update(ASCEND_RT_VISIBLE_DEVICES="6,7", COMPARE_NO_MTP=arm,
                       ELASTIC_CANDIDATE="1", TASK_QUEUE_ENABLE="0" if arm == "candidate" else "1",
