...
for entry in entries:
    ...
    with sqlite3.connect(db) as c:
        ...
        resolved_source = Path(metadata["source_path"]).resolve()
        if source.is_dir():
            assert resolved_source.parent == source.resolve()
            assert (
                resolved_source.name.startswith("msprof_")
                and resolved_source.suffix == ".db"
            )
        else:
