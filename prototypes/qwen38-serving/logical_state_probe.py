...
try:
    ...
    while True:
        ...
        time.sleep(2)
    for mode in ["native", "padded-none", "full"]:
        rpc("set_prefill_policy", mode)
        rpc("arm_logical", mode, 512)
        receipt["rows"].append(dict(policy=mode, **request(url, prompts[512], 1)))
    receipt["status"] = "COMPLETE_DIAGNOSTIC"
