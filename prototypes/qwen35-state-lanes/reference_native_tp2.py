...
def main():
    ...
    llm = LLM(model=model, dtype='bfloat16', tensor_parallel_size=2,
        distributed_executor_backend='mp', worker_cls='native_worker.Worker',
        max_model_len=4096, max_num_seqs=2, max_num_batched_tokens=2048,
        kv_cache_memory_bytes=2 << 30, enforce_eager=True,
        enable_prefix_caching=False, seed=17,
    ...
    def run(tokens, count, ignore_eos):
        result = llm.generate([{'prompt_token_ids':tokens}], SamplingParams(
            temperature=0, max_tokens=count, ignore_eos=ignore_eos, logprobs=5), use_tqdm=False)[0].outputs[0]
        return {'prompt':tokens, 'token_ids':list(result.token_ids), 'text':result.text,
                'logprobs':[{str(k):v.logprob for k,v in row.items()} for row in result.logprobs]}
    tokens = [9707,11,358,1079,264,1786,13]
    base = run(tokens,12,True)
    continuation = run(tokens+base['token_ids']+[271],8,True)
    tokenizer = AutoTokenizer.from_pretrained(model, local_files_only=True)
    chats=[]
    for question in ['What is 2 + 2? Answer with only the number.',
    ...
    receipt={'base':base,'continuation':continuation,'chats':chats,
             'scope':'independent pinned native TP2 BF16 MTP2 eager runner, raw fixtures ignore EOS; chat honors EOS'}
    Path(os.environ['CAPSULE'],'reference.json').write_text(json.dumps(receipt,indent=2)+'\n')
