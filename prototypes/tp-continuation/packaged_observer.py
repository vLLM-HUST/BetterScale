...
class PackagedObserver:
    ...
    def donor_receipt(self):
        ...
        r = self.model_runner
        producer, metadata = getattr(r, "_async_decode", (None, None))
        ...
        return dict(
            ...
            * r.vllm_config.parallel_config.tensor_parallel_size
            + self.rank,
            ...
            target_replays=getattr(
                getattr(r.model, "_decode_pair", None), "replays", None
            ),
            preparation_replays=getattr(producer, "sequence", None),
            preparation_banks=len(producer.slots) if producer is not None else 0,
            metadata_entries=len(metadata.entries) if metadata is not None else 0,
            metadata_replays=getattr(metadata, "replays", None),
        )
