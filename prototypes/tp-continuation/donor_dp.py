...
def rank_main(args, dp_rank, barrier):
    os.environ.update(
        ...
        VLLM_DP_SIZE=str(args.donor_dp),
        VLLM_DP_MASTER_IP="127.0.0.1",
        VLLM_DP_MASTER_PORT="30651",
        DONOR_DP_OUTPUT=str(args.output),
    )
    os.environ["VLLM_ASCEND_ENABLE_FLASHCOMM1"] = "1" if args.tp > 1 else "0"
    os.environ["DONOR_PINGPONG"] = "1" if args.pingpong else "0"
    os.environ["DONOR_PINGPONG_CAPTURED_COPY"] = (
        "1" if args.pingpong_captured_copy else "0"
    )
    ...
    local_seats = (
        (4 if args.tp > 1 else 2)
        if args.packaged or not args.real
        else 16 // args.donor_dp
    )
    ...
    if args.dp_full:
        ...
        capture_sizes += [
            n for n in (132, 264, 516, args.budget) if 12 < n <= args.budget
        ]
        capture_sizes = sorted(set(capture_sizes))
    config = dict(
        model=os.environ["PROBE_MODEL"],
        load_format="auto" if args.real else "dummy",
        tensor_parallel_size=args.tp,
        enable_expert_parallel=True,
        quantization="ascend",
        dtype="bfloat16",
        max_model_len=15104 if args.packaged and args.tp > 1 else 16384,
        max_num_batched_tokens=args.budget,
        max_num_seqs=local_seats,
        kv_cache_memory_bytes=int((args.kv_gib or 3) * 1024**3),
        enable_prefix_caching=False,
        skip_tokenizer_init=True,
        seed=123,
        block_size=128,
        worker_extension_cls=(
            "dp_full.DPFullWorker" if args.dp_full else "donor_dp_worker.DonorDPWorker"
        ),
        compilation_config=dict(
            cudagraph_mode="FULL" if args.dp_full else "FULL_DECODE_ONLY",
    ...
    if args.packaged:
        ...
        assert not any(
            (
                args.pingpong,
                args.pingpong_sources,
                args.pingpong_continuous,
                args.shadow_decode,
                args.pingpong_study,
                args.dp_shadow,
            )
        ...
        if args.tp > 1:
            config["compilation_config"] = dict(
                cudagraph_mode="FULL",
                cudagraph_capture_sizes=[24, args.budget],
                max_cudagraph_capture_size=args.budget,
            )
        config["worker_cls"] = (
            "packaged_oracle.AcceptanceWorker"
            if args.packaged_oracle
    ...
    (args.output / f"dp{dp_rank}-protocol.json").write_text(
        json.dumps(
            dict(
                shadow_decode=args.shadow_decode,
                shadow_metadata=args.shadow_metadata,
                producer_oracle=args.shadow_decode_verify,
                target_oracle=args.pingpong_shadow,
                hccl_deterministic=os.environ.get("HCCL_DETERMINISTIC"),
                split_draft=args.split_draft,
                worker_continuous=args.worker_continuous,
                draft_oracle=os.environ.get("DRAFT_GRAPH_SHADOW") == "1",
                decode_only=args.decode_only,
                study=args.pingpong_study,
            ),
            indent=2,
        )
    )
    ...
    if args.shadow_decode:
        ...
        llm.collective_rpc(
            "enable_shadow_decode",
            args=(
                args.shadow_decode_verify,
                args.shadow_metadata,
            ),
        )
    if args.split_draft:
    ...
    def wave(label, lengths, output, profile=False, observe=True):
        ...
        result = llm.generate(
            [
                dict(prompt_token_ids=[17 + (dp_rank * len(lengths) + i) % 4] * n)
                for i, n in enumerate(lengths)
            ],
            SamplingParams(
                temperature=0, max_tokens=output, ignore_eos=True, detokenize=False
            ),
        )
        ...
        record = dict(
            ...
            elapsed=elapsed,
            outputs=[list(x.outputs[0].token_ids) for x in result],
            profile=profile,
        )
        rows.append(record)
        (args.output / f"dp{dp_rank}-results.json").write_text(
    ...
    if args.pingpong_study:
        assert (
            ...
            and not (args.dp_shadow or args.pingpong_shadow)
        )
            ...
            for policy in (
                ...
                if args.worker_continuous
                    ...
                    if args.split_draft
                        ...
                        ("native", "cut", "pair", "producer", "metadata")
                        if args.shadow_metadata
                        else ("native", "cut", "sources", "pair")
                    )
                )
            ):
                barrier.wait(timeout=120)
        ...
        for repeat in range(2):
            ...
            if not args.decode_only:
                ...
                wave(
                    f"skew{repeat}",
                    [
                        8192 if dp_rank * (8 // args.donor_dp) + i == 0 else 256
                        for i in range(8 // args.donor_dp)
                    ],
                    16,
                )
    elif not args.real:
        ...
        wave(
            "dummyskew",
            [args.budget * 2 + 17 if dp_rank == 0 else 32]
            * (1 if single_card else 8 // args.donor_dp),
            16,
        )
    ...
    if args.quality_requests:
        assert (
            args.real and not args.dp_shadow and (args.donor_dp == 8 or args.packaged)
        )
    ...
    (args.output / f"dp{dp_rank}-receipt.json").write_text(
...
def main(args):
    ...
    assert not any(
        (
            ...
            args.cpu_qli,
            args.draft_graph,
            args.cross_step_bounds,
        )
    )
    assert (args.donor_dp * args.tp == 8 and args.donor_dp in (1, 2, 4, 8)) or (
        args.tp in (1, 2, 4)
        and args.donor_dp in (1, 2, 4)
        and args.donor_dp * args.tp <= 8
        and (args.dp_full or args.tp > 1)
        and not args.real
    )
    args.output.mkdir(parents=True, exist_ok=True)
    ctx = mp.get_context("spawn")
    barrier = ctx.Barrier(args.donor_dp)
    processes = [
        ctx.Process(
            target=rank_entry, args=(args, i, barrier), name=f"donor-dp-client-{i}"
        )
        for i in range(args.donor_dp)
    ]
    ...
    while any(p.is_alive() for p in processes):
        ...
        if failed or barrier.broken:
            barrier.abort()
            raise RuntimeError(
                f"DP client failed or aborted barrier: {[(p.name,p.exitcode) for p in failed]}"
            )
        for p in processes:
            p.join(timeout=0.2)
    ...
    if args.quality_requests:
        ...
        results = [
            r
            for i in range(args.donor_dp)
            for r in json.loads((args.output / f"dp{i}-quality.json").read_text())
        ]
        ...
        (args.output / "quality-result.json").write_text(
            json.dumps(
                dict(
                    ...
                    scope="Retained 32 OpenCompass LongBench English retrieval items; native DP8",
                    requests=results,
                ),
                indent=2,
            )
        )
    (args.output / "dp-completion.json").write_text(
        json.dumps(
            dict(
                dp=args.donor_dp, tp=args.tp, exitcodes=[p.exitcode for p in processes]
            ),
            indent=2,
        )
    )
