...
capsule="$repo/runs/gmm-batch-control-$stamp"
...
printf '%s\n' "$capsule"
exec "$runtime/bin/python" "$capsule/source/launch.py" --devices "$devices" --output "$capsule/run" -- "$runtime/bin/python" "$capsule/source/device/gmm_batch_probe.py"
