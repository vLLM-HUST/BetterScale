...
capsule="$repo/runs/gmm-layout-control-$stamp"
...
printf '%s\n' "$capsule"
exec "$runtime/bin/python" "$capsule/source/launch.py" --devices "$devices" --output "$capsule/run" -- "$runtime/bin/python" "$capsule/source/device/gmm_layout_probe.py"
