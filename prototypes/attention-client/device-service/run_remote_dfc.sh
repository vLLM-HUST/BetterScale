...
capsule="$repo/runs/remote-dfc-control-$stamp"
...
printf '%s\n' "$capsule"
exec "$runtime/bin/python" "$capsule/source/launch.py" --devices "$devices" --output "$capsule/run" -- "$runtime/bin/python" "$capsule/source/device/remote_dfc_control.py" --devices "$devices" --out "$capsule/run/measurements"
