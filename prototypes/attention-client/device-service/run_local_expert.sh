...
capsule="$repo/runs/local-expert-control-$stamp"
...
printf '%s\n' "$capsule"
exec "$runtime/bin/python" "$capsule/source/launch.py" --devices "$devices" --output "$capsule/run" -- "$runtime/bin/python" "$capsule/source/device/local_expert_probe.py"
