...
if [[ "${NEXT_LEAF:-0}" == "1" ]]; then
  ...
  exec "$runtime/bin/python" "$capsule/source/device/admit_subset.py" --devices "$devices" --output "$capsule/run" -- timeout 120 "$runtime/bin/python" "$capsule/source/next/next_leaf.py"
fi
