...
class Bank:
    def __init__(self, session, rows):
        ...
        self.prob_storage = torch.zeros((32, 10), dtype=torch.bfloat16, device="npu")
        self.probs = self.prob_storage[:rows]
        self.raw = (
            None
            if session.route_pull
            else torch.empty((rows * 10, 2048), dtype=torch.bfloat16, device="npu")
        )
...
class Session:
    def __init__(self):
        ...
        self.collect = self.kernels.load(
            "neural_collect_reduce" if self.route_pull else "neural_collect"
        )
