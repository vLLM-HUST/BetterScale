...
class AttentionProbeWorker(NPUWorker):
    def compile_or_warm_up_model(self):
        ...
        def forward(*args, **kwargs):
            ...
            def visit(obj):
                if isinstance(obj, torch.Tensor):
                    ...
                    if key not in unique:
                        view = torch.empty(0, dtype=torch.uint8, device=obj.device).set_(
            ...
            graph_replays = 0
            if os.environ.get('ATTENTION_GRAPH_PROBE') == '1' and got.shape[0] not in self.graph_shapes:
                ...
                try:
                    ...
                    for _ in range(3):
                        ...
                        assert all(torch.equal(v, ref) for v, ref in unique.values()), 'graph KV differs'
                        graph_replays += 1
                finally:
                    torch.npu.synchronize()
                    ctx.moe_layer_index = exit_moe_index
            self.checks.append(dict(rows=got.shape[0], graph_replays=graph_replays, output_exact=exact,
                                    kv_exact=True, max_abs_error=error))
            Path(os.environ['ATTENTION_PROBE_OUT']).write_text(json.dumps(self.checks, indent=2))
            return got
