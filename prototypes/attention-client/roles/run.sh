...
capsule="$repo/runs/independent-roles-$stamp"
mkdir -p "$capsule/source/roles" "$capsule/source/device" "$capsule/source/joint" "$capsule/build"
...
export PYTHONPATH="$capsule/source/roles:$capsule/source/device:$capsule/source/joint:$capsule/source${PYTHONPATH:+:$PYTHONPATH}"
...
socket_dir="/tmp/expert-roles-$stamp"
printf '%s\n' "$socket_dir" > "$capsule/socket-directory.txt"
git -C "$repo" rev-parse HEAD > "$capsule/base-commit.txt"
printf '%s\n' "$capsule"
if [[ "${ROLE_EOF_ONLY:-0}" == "1" ]]; then
  export ASCEND_RT_VISIBLE_DEVICES="$devices" LOCAL_EXPERT_RESULT="$capsule/eof-result.json"
  exec "$runtime/bin/python" "$capsule/source/device/admit_subset.py" --devices "$devices" --output "$capsule/run" -- "$runtime/bin/python" "$capsule/source/roles/eof_probe.py"
fi
exec "$runtime/bin/python" "$capsule/source/device/admit_subset.py" --devices "$devices" --output "$capsule/run" -- "$runtime/bin/python" "$capsule/source/roles/launch.py" --devices "$devices" --directory "$socket_dir" --artifacts "$capsule/roles"
