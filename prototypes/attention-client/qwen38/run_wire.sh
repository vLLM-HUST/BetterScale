...
DEVICES=${1:-1,3,4,5,6}
exec "$PY" "$CAP/admission-helper/admit_subset.py" --devices "$DEVICES" --output "$CAP/run" --wait-seconds "${QWEN38_WAIT_SECONDS:-1200}" -- "$PY" "$CAP/source/launch_wire.py" --devices "$DEVICES" --directory "/tmp/q38-wire-$$" --build "${QWEN38_BUILD:-$ROOT/runs/qwen38-server-build-20260917T0348-lifetime}"
