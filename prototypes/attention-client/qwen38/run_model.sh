...
if (( $# )); then shift; fi
exec "$PY" "$CAP/admission-helper/admit_subset.py" --devices "$DEVICES" --output "$CAP/run" --wait-seconds "${QWEN38_WAIT_SECONDS:-1200}" -- "$PY" "$CAP/source/launch_model.py" --devices "$DEVICES" --directory "/tmp/q38-model-$$" --artifacts "$CAP/roles" --build "${QWEN38_BUILD:-$ROOT/runs/qwen38-server-default-20260918}" "$@"
