...
export PYTHONPATH="$CAP/source:$ROOT/prototypes/attention-client/roles:${QWEN38_OVERLAY:-$ROOT/runs/qwen38-native-runtime-20260917/overlay}:${PYTHONPATH:-}"
...
if (( $# )); then shift; fi
exec "$PY" "$CAP/admission-helper/admit_subset.py" --devices "$DEVICES" --output "$CAP/run" --wait-seconds "${QWEN38_WAIT_SECONDS:-1200}" -- "$PY" "$CAP/source/launch_model.py" --devices "$DEVICES" --directory "/tmp/q38-model-$$" --artifacts "$CAP/roles" --build "${QWEN38_BUILD:-$ROOT/runs/qwen38-server-build-20260917T0348-lifetime}" "$@"
