...
import argparse
p=argparse.ArgumentParser();p.add_argument('root',type=Path);p.add_argument('--export',action='store_true');p.add_argument('--eager-markers',action='store_true');p.add_argument('--max-holdout-us',type=float,default=50);a=p.parse_args()
...
(clock/'markers.json').write_text(json.dumps(dict(contract='unique identical provider opName/opType/group/count; no timestamp-nearest pairing; end-affine candidate display only',eager_markers=a.eager_markers,common_count=len(common),selected=[list(k) for k in keys]),indent=2))
