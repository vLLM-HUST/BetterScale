...
import argparse
p=argparse.ArgumentParser();p.add_argument('root',type=Path);p.add_argument('--export',action='store_true');a=p.parse_args()
...
for rank in range(8):
 p=next((root/'profile').glob(f'rank{rank}_*/ASCEND_PROFILER_OUTPUT/*.db'));c=sqlite3.connect(p)
 rows=c.execute('select n.value,t.value,g.value,o.count,o.startNs,o.endNs from COMMUNICATION_OP o join STRING_IDS n on n.id=o.opName join STRING_IDS t on t.id=o.opType join STRING_IDS g on g.id=o.groupName').fetchall();c.close()
...
(clock/'markers.json').write_text(json.dumps(dict(contract='unique identical provider opName/opType/group/count; no timestamp-nearest pairing; end-affine candidate display only',common_count=len(common),selected=[list(k) for k in keys]),indent=2))
...
if a.export:
    args=[str(tool/'build/traceloom'),'export-perfetto',str(root/'analysis/rank0.db'),
          '--output',str(root/'analysis/target-draft-tp8-end-aligned.json.gz'),
    ...
    with (root/'analysis/export.log').open('w') as f:
        subprocess.run(args,stdout=f,stderr=subprocess.STDOUT,check=True,timeout=300)
