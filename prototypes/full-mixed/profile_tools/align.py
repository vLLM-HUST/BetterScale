...
import argparse
p=argparse.ArgumentParser();p.add_argument('root',type=Path);p.add_argument('--export',action='store_true');p.add_argument('--eager-markers',action='store_true');p.add_argument('--max-marker-count',type=int);p.add_argument('--max-holdout-us',type=float,default=50);p.add_argument('--ranks',type=int,choices=range(2,9),default=8);p.add_argument('--name',default='target-draft-tp8-end-aligned');a=p.parse_args()
...
if a.export:
    output=root/'analysis'/f'{a.name}.json.gz'
    partial=root/'analysis'/f'{a.name}.partial.json.gz'
    args=[str(tool/'build/traceloom'),'export-perfetto',str(root/'analysis/rank0.db'),
          '--output',str(partial),
          '--distributed-clock-model',str(clock/'models.jsonl')]
    for rank in range(a.ranks):args+=['--distributed-rank',f'{rank}={root}/analysis/rank{rank}.db']
