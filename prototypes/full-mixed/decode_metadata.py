...
class DeviceOnly(TorchDispatchMode):
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        kwargs = kwargs or {}
        tensors = [v for v in tree_flatten((args, kwargs))[0] if isinstance(v, torch.Tensor)]
        ...
        assert not (target == self.device_type and 'cpu' in devices), f'Unbanked metadata ingress: {func}'
        assert not (target == 'cpu' and self.device_type in devices), f'Metadata readback: {func}'
        assert not (func._schema.name == 'aten::_local_scalar_dense' and self.device_type in devices), f'Metadata scalar readback: {func}'
        if func._schema.name == 'aten::copy_' and len(args) > 1:
            assert args[0].device.type == args[1].device.type, f'Unbanked metadata copy: {func}'
...
class DecodeMetadata:
    ...
    def build(self, *args, **kwargs):
        ...
        if (not self.enabled or args or self.producer.active is None or not r._cross_step_bounds.admitted
                or kwargs.get('for_cudagraph_capture') or kwargs.get('ubatch_slices') is not None):
