...
p=argparse.ArgumentParser();p.add_argument('--output',type=Path,required=True);p.add_argument('--mode',choices=['FULL','NONE'],default='FULL');p.add_argument('--tp',type=int,default=2);p.add_argument('--spec',action='store_true');p.add_argument('--budget',type=int,default=256);a=p.parse_args()
a.output.mkdir(parents=True,exist_ok=True)
from vllm import LLM,SamplingParams
config=dict(model='/data/shared_models/DeepSeek-V4-Flash-0731-w8a8',load_format='dummy',
 ...
 gpu_memory_utilization=.35,num_gpu_blocks_override=4096,enable_prefix_caching=False,skip_tokenizer_init=True,seed=123,
...
if a.spec:
 config['hf_overrides'].update(num_nextn_predict_layers=1,compress_ratios=[0,0,4,128,0,0,0],dspark_target_layer_ids=[1,2,3])
...
results=[]
for lengths in [[64,64,64,64],[129,17],[7,128,33],[256,23,5,9],[513,257,17],[1025]]:
