...
def client(rank, barrier):
    ...
    llm.collective_rpc('collect_reference')
    barrier.wait(timeout=600)
    ...
    if os.environ.get('QWEN_PROMPTS') == 'language':
        ...
        prompt_ids = [198] * (32-len(prompt_ids)) + prompt_ids
    else:
    ...
    result = llm.generate([dict(prompt_token_ids=prompt_ids)],
                          SamplingParams(temperature=0, max_tokens=7, ignore_eos=True, detokenize=False))
    barrier.wait(timeout=600)
    receipts = llm.collective_rpc('run_owned', args=(prompt_ids,))
