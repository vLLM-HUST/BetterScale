...
def preprocess_kernel(
    ...
    off = token[:, None, None] * XS + head * 128 + d
    ...
    tl.store(Q + dest, qy, mask)
    tl.store(K + dest, ky, mask)
    vd = tl.arange(0, 4096)[None, :]
    vm = (token[:, None] < T) & (vd < 3072)
    value = tl.load(X + token[:, None] * XS + 2048 + vd, vm, other=0)
    tl.store(V + token[:, None] * 3072 + vd, value, vm)
    gh = tl.arange(0, 32)[None, :]
    gm = (token[:, None] < T) & (gh < 24)
    a = tl.load(A + token[:, None] * AS + gh, gm, other=0).to(tl.float32)
    b = tl.load(B + token[:, None] * BS + gh, gm, other=0).to(tl.float32)
...
def preprocess(x, a, b, log, bias):
    t = x.shape[0]
    assert x.shape == (t, 5120) and 0 < t <= 2048
    assert a.shape == b.shape == (t, 24)
    ...
    assert x.stride(-1) == 1
    q = torch.empty((1, t, 8, 128), dtype=x.dtype, device=x.device)
