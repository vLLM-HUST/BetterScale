...
int main() {
  ...
  const auto size=plans.size();
  for(int wave=0;wave<1000;++wave) {
    fixture(); int next=plan_finish(); assert(next>=0);
    assert(plan_replace(id,next)==0 && plans.size()==size);
    assert(plan_workspace(id)==(16ULL<<20)+4096);
    assert(plan_clone(next)==-1);
  }
  fixture(); int incompatible=plan_finish(); plans[incompatible]->blocks=7;
  assert(plan_replace(id,incompatible)==-2);
  assert(plan_release(incompatible)==0);
  assert(plan_replace(id,id)==-1);
