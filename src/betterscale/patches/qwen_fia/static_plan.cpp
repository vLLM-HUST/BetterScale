...
extern "C" int plan_replace(int destination, int temporary) {
  if(destination<0 || temporary<0 || destination==temporary ||
     size_t(temporary)+1!=plans.size() || size_t(destination)>=plans.size() ||
     !plans[destination] || !plans[temporary]) return -1;
  auto &a=*plans[destination], &b=*plans[temporary];
  if(a.fn!=b.fn || a.blocks!=b.blocks || a.fd!=b.fd) return -2;
  plans[destination]=std::move(plans.back()); plans.pop_back(); return 0;
}
