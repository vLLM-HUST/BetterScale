...
Imported without touching native runtime state. The DSV4 lifecycle calls these functions;
...
def determine_available_memory(self, native, *, capture_trial, retire_trial):
    ...
    try:
        ...
        graph_bytes = capture_trial(self)
        assert graph_bytes > 0
        snapshot(self, "trial_after_capture", trial_graph_bytes=graph_bytes)
    finally:
        retire_trial(self)
    ...
    if persistent_growth >= 256 * (1 << 20):
        raise RuntimeError(f"Trial KV retirement retained {persistent_growth} bytes")
    safety = GiB
    budget = int(
