...
def pins():
    return json.loads(resources.files('strengthen_dsv4').joinpath('pins.json').read_text())


@lru_cache(maxsize=1)
def check_runtime():
    expected=pins(); actual={}; failures=[]
    for name, version in expected['versions'].items():
        try:
            actual[name]=metadata.version(name)
        except metadata.PackageNotFoundError:
            failures.append(f'{name}: not installed'); continue
    ...
    for item in expected['source_files']:
        try:
            ...
            failures.append(item['path']+': missing'); continue
        if digest!=item['sha256']:
            failures.append(item['path']+': differs from the qualified pin')
    if failures:
        raise RuntimeError('Incompatible donor environment; no patches applied:\n'+ '\n'.join(failures))
    return dict(versions=actual, pins=expected['commits'], checked_source_files=len(expected['source_files']))
