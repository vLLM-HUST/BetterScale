...
class HostSourceSlots:
    ...
    def scope(self):
        ...
        if self.used[bank]:
            self.events[
                bank
            ].synchronize()  # actual DMA source reuse, not a device-only wait
...
def install(worker):
    ...
    for name in (
        ...
        fields.append(
            HostField(carrier, "cpu", "np" if hasattr(carrier, "np") else None)
        )
