...
def stable_verification(runner, schedule, counts):
    ...
    return (
        ...
        and all(len(schedule.scheduled_spec_decode_tokens.get(rid, ())) == 5 for rid in ids)
        and runner.valid_sampled_token_count_gpu is not None
        and runner._draft_token_ids is not None
        and all(runner.input_batch.num_computed_tokens_cpu[i] >= runner.input_batch.num_prompt_tokens[i]
...
class CrossStepBounds:
    ...
    def correct_bounds(self, n):
        ...
        if not self.admitted or not bool(((upper > 0) & (upper <= r.max_model_len)).all()):
    ...
    def reference_begin(self, context):
        ...
        assert bool((upper[:n] >= exact).all()), 'CPU authorization underestimates device progress'
